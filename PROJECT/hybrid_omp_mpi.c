...
#define N 1280 // Total number of rows
#define M 640 // Total number of columns


/* COMP 605 Final Project - 2D Domain Decomposition using Hybrid OpenMP+MPI
                Akhil Perimbeti - May 17th, 2022  */
...
int main(int argc, char* argv[])
{
    ...
    if ((M*N) % nprocs != 0 || nprocs < 2) {
        printf("\n ERROR: The domain must be evenly divided by the total number of processors. Minimum amount of processors: 2 \n");
        MPI_Abort( MPI_COMM_WORLD, 1 ); // If the dimensions or processors do not allow for an even number of 'blocks'
    }

    if (nprocs%2 == 1 || (M*N)%2 == 1 ){
    ...
    if (nprocs > 4){ // More than 4 processors
    ...
    if (nprocs == 2) { // if exactly 2 processors
            nb_cols = nb_cols - 1;
    }
    ...
    do {
        ...
        /* JACOBI Iteration! - Parrallel OpenMP implementation*/
        ...
        #pragma omp parallel for num_threads(num_threads) private(i,j) shared(xlocal, xnew)
        ...
        #pragma omp barrier     // Thread synchronization after performing the Jacobi iteration

        //normalized difference calculation between xnew and xlocal (current values - prev. iteration values)
        #pragma omp parallel for num_threads(num_threads) private(i,j) shared(xlocal, xnew) reduction(+:diffnorm)
         for (i=1; i<=nb_rows-2; i++) {
            for (j=1; j<=nb_cols-2; j++) {
        ...
        #pragma omp barrier    // Thread synchronization after calculating the Normalized Error between iterations
        ...
        #pragma omp barrier  // Thread synchronization after updating the local array with the new averaged values.
        ...
        gdiffnorm = sqrt( gdiffnorm );              // calculate global error
    ...
    MPI_Barrier(MPI_COMM_WORLD); // Synchronize all processes - wait till finished before stopping the timer
    ...
    if (rank == 0) {
        ...
        printf("--------------------------------------------------\n");
        printf("        Number of Threads (OpenMP): %d \n", num_threads);
        printf("--------------------------------------------------\n");
        printf("                    RESULTS:                \n");
        printf("--------------------------------------------------\n");
